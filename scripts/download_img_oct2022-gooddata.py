...
if __name__ == '__main__':
    img_df = pd.read_csv('/Users/travistang/Documents/TorchScene/data/csv/oct2022-gooddata.csv')
    img_df = img_df.dropna(subset=['restaurant_photo_url'])
    ...
    for loop in range(10):
        ...
        url_sel_list = img_df['restaurant_photo_url']
        merchant_sel_list = img_df['outlet_id']

        folder_name = "data/images/oct2022-gooddata/"
        pool.starmap(download_image, zip(url_sel_list, merchant_sel_list, repeat(folder_name)))
        duration = datetime.now() - start
        print(f'loop {loop} completed')
